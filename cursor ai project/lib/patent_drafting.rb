...
module PatentDrafting
  ...
  def self.build_mermaid(components, steps)
    ...
    component_nodes.each_with_index do |node, idx|
      ...
      lines << "  C#{idx + 1}[\"#{sanitize_mermaid_label(node[:label])}\"]"
      lines << "  #{connector} --> C#{idx + 1}" if idx >= 1
    end

    lines << "  D1 --> S1"
    step_nodes.each_with_index do |node, idx|
      lines << "  #{node[:id]}[\"#{sanitize_mermaid_label(node[:label])}\"]"
      lines << "  S#{idx + 1} --> S#{idx + 2}" if idx + 1 < step_nodes.length
    end

    lines << "  S#{step_nodes.length} --> OUT[\"Outcome / Result\"]"
    lines
      .join("\n")
      .gsub(/[ ]{2,}/, " ")
  end

  def self.sanitize_mermaid_label(label)
    text = label.to_s.gsub(/[\r\n]+/, " ").strip
    # Remove characters that frequently break Mermaid node labels.
    text = text.gsub(/["\[\]\{\}\(\)]/, " ")
  ...
  def self.build_html(application_type, title, summary, how_it_works, key_components, process_steps, use_cases, claims, diagram_mermaid, external_sources)
    ...
    css = <<~CSS
      ...
      .diagram-wrap { border: 1px solid #ddd; border-radius: 6px; padding: 10px; background: #fff; }
    ...
    html = <<~HTML
      ...
      <html>
        ...
        <body>
          ...
          <h2>FIG. 1 Block Diagram</h2>
          <div class="diagram-wrap">
            <div class="mermaid">
#{CGI.escapeHTML(diagram_mermaid)}
            </div>
          </div>
          ...
          <h2>Block Diagram Source (Fallback)</h2>
          <p class="muted">If rendering is unavailable in your viewer, use this Mermaid source to regenerate the figure.</p>
